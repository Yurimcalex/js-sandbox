class PageChanger {
  constructor() {
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      const srcInd = this.paths.indexOf(src);

      this._createCounterUI(srcInd, this.paths.length - 1);
      this._createHeadingUI(src);

      prev.addEventListener('click', () => this.change(srcInd, '-'));
      next.addEventListener('click', () => this.change(srcInd, '+'));
    }); 
  }
  ...
  change(currPathInd, dir) {
    ...
    if (currPathInd === this.paths.length -1  && dir === '+') return;
    ...
    window.location.assign(`/${this.paths[nextPathInd]}`);
  }
